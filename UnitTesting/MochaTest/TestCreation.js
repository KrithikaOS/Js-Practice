...
function checkEmailExists(userEmail) {
    ...
    makeAjaxCall(url, {
        ...
        async: true
    }).then(function (checkEmailResponse) {
...
function getQuestionsFromTag() {
    ...
    makeAjaxCall(url, {
        method: 'GET',
        async: true
    }).then(function (getQuestionsResponse) {
...
function sendSelectedQuestions(items) {
    // var items = document.getElementsByName('selectedQuestions');
    var selectedQuestions = [];
    var j = 0;
    for (var i = 0; i < items.length; i++) {
        if (items[i].type == 'checkbox' && items[i].checked == true) selectedQuestions[j++] = items[i].value;
    }
...
function getTime(timeSelected) {
    //var timeSelected = document.getElementById("inputTime").value;
    if (timeSelected === "") {
...
function createTest() {
    ...
    var selectedQuestions = sendSelectedQuestions(document.getElementsByName('selectedQuestions'));
    var testDuration = getTime(document.getElementById("inputTime").value);
    ...
    makeAjaxCall(url, {
        ...
        async: true
    }).then(function (createTestResponse) {
        alert("Test Created");
